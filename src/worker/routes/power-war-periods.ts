...
import { powerWarPeriods } from '../db/schema';
...
app.get('/', async (c) => {
  ...
  const rows = await db.select().from(powerWarPeriods).orderBy(asc(powerWarPeriods.startDate));
...
app.patch('/:id', async (c) => {
  ...
  const [updated] = await db
    .update(powerWarPeriods)
    .set(data)
    .where(eq(powerWarPeriods.id, id))
    .returning();
...
app.delete('/:id', async (c) => {
  ...
  const db = getDb(c.env.DB);
  await db.delete(powerWarPeriods).where(eq(powerWarPeriods.id, id));
